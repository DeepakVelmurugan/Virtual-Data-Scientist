...
def neuralNet(dt):
    ...
    if type_of_model == "Classification":
        model = MLPClassifier(hidden_layer_sizes=hidden_layers,activation=activation,solver=solver,alpha=alpha)
    else:
        model = MLPRegressor(hidden_layer_sizes=hidden_layers,activation=activation,solver=solver,alpha=alpha)
    return model
...
def MLmodels():
    if request.method == "POST":
        ...
        if(os.path.isfile("csv/train.csv") and os.path.isfile("csv/val.csv")): #if oridinary train-test split
            ...
            if test_dt["type"] == "NeuralNets":
                MAE  = mean_absolute_error(y_val,pred)
                return render_template("MLmodels.html",scores=[MAE])
            if(test_dt["evaluation_metric"] == "MAE"):       #Mean absolute error
                MAE  = mean_absolute_error(y_val,pred)
                return render_template('MLmodels.html',scores=[MAE])
            ...
            for fold in range(no_of_folds):
                ...
                pred = model.predict(X_val)
                if test_dt["type"] == "NeuralNets":
                    loss_list.append(mean_absolute_error(y_val,pred))
                if(test_dt["evaluation_metric"] == "MAE"):       #Mean absolute error               
                    loss_list.append(mean_absolute_error(y_val,pred))
...
def upload_file():
    if request.method == 'POST':
        try:       
           ...
           return "Error ,please check type of file and index column name"
    else:
        return render_template('upload.html')
